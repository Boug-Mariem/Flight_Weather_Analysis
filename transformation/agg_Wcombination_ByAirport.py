...
def build_weather_airport_delay_dataset(df):
    # CAS 2 → départ uniquement
    df_dep_only = df[df['delay_scenario'] == 2][
        ['Dep_Airport', 'weather_combination_dep', 'Dep_Delay']
    ].rename(columns={
        'Dep_Airport': 'airport',
    ...
    df_arr_only = df[df['delay_scenario'] == 3][
        ['Arr_Airport', 'weather_combination_arr', 'Arr_Delay']
    ].rename(columns={
        'Arr_Airport': 'airport',
    ...
    df_both_arr = df[df['delay_scenario'] == 1][
        ['Arr_Airport', 'weather_combination_arr', 'delay_change']
    ].rename(columns={
        'Arr_Airport': 'airport',
        'weather_combination_arr': 'weather_combination',
        'delay_change': 'delay'
...
def compute_airport_weather_agg(df):
    df['is_delay'] = (df['delay'] > 0).astype(int)
    agg = df.groupby(['airport', 'weather_combination'],observed=True).agg(
        sum_delay=('delay', 'sum'),
...
def get_agg_Wcombination_processed_dates(engine):
    try:
        query = 'SELECT DISTINCT "FlightDate" FROM agg_wcombination_byairport_processed_dates'
        df = pd.read_sql(query, engine)
        return set(pd.to_datetime(df["FlightDate"]).dt.date)
    except Exception as e:
        print("agg_wcombination_byairport_processed_dates table not found, first run.")
        return set()

def create_agg_Wcombination_table(engine):
    query = text("""
    CREATE TABLE IF NOT EXISTS agg_delay_by_wcombination_byairport (
        airport TEXT,
        weather_combination TEXT,
        sum_delay FLOAT,
        flight_count INT,
        delay_count INT,
        PRIMARY KEY (airport, weather_combination)
    );
    """)
...
def upsert_agg_Wcombination(batch_df, engine):
    metadata = MetaData()
    table = Table("agg_delay_by_wcombination_byairport", metadata, autoload_with=engine)
    records = batch_df.to_dict(orient="records")
    stmt = insert(table).values(records)
    stmt = stmt.on_conflict_do_update(
        index_elements=["airport","weather_combination"],
        set_={
...
def agg_delay_by_wcombination_byairport():
    engine = get_engine()
    # chercher les dates non encore traiter dans agg_wcombination_byairport_processed_dates
    global_table_dates= get_gold_table_dates(engine)
    agg_wcombination_byairport_processed_dates= get_agg_Wcombination_processed_dates(engine)
    
    new_dates = global_table_dates-agg_wcombination_byairport_processed_dates
    if not new_dates:
        print("Pas de nouvelle date pour agg_delay_by_wcombination_byairport ")
        return

    # Apporter les donner de gold table non encore traitr dans agg_delay_by_wcombination_byairport
    df = load_gold_table(engine,new_dates)

    dataset=build_weather_airport_delay_dataset(df)
    agg= compute_airport_weather_agg(dataset)
    ...
    print("mise a jour de agg_delay_by_wcombination_byairport ")
    print("Nombre de lignes :", agg.shape)

    dates_df = pd.DataFrame({'FlightDate': list(new_dates)})
    dates_df.to_sql(
            "agg_wcombination_byairport_processed_dates",
            engine,
            if_exists="append",
            index=False
        )
    dates_df = sorted(new_dates)
    print("mise a jour de agg_wcombination_byairport_processed_dates avec dates : ", dates_df)
    ...
    query = text("""
        SELECT *
            FROM (
                ...
                FROM agg_delay_by_wcombination_byairport
...
def Wcombinations_byairport():
    agg_delay_by_wcombination_byairport()
    engine=get_engine()
    df_result = compute_final_metrics(engine)
    print("le meilleur aéroport dans chaque condition météo:")
    print(df_result)
